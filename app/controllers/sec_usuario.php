...
class sec_usuario extends My_Controller{
    ...
    public function modificar($rut){
        ...
        if($this->form_validation->run()===FALSE){
            ...
            if($this->input->post("cboTipo1")!=$detalle['usuario']['idTipousuario']){
                //echo "PASO";
                $this->usuarios_model->del_permisos($rut);
                $permisos=$this->usuarios_model->get_permisos($rut);
              
                foreach($permisos as $valor){
                    echo $valor['idPermiso']." ";
                    $acciones=$this->usuarios_model->get_acciones($valor['idPermiso'],'',"pa.idPermisoaccion");
                    
                    $this->usuarios_model->set_permisos($rut,$acciones);
                }
                $detalle['usuario']['idTipousuario']=$this->input->post("cboTipo1");
                
            }
           $detalle['usuario']['idEstadousuario']=$this->input->post("cboEstado1");
        } 
        $detalle['estado']=$this->common_model->get_estado_usuario();
        $detalle['tipo']=$this->common_model->get_tipo_usuario();
     ...
     public function eliminar($id_usuario){
        $this->usuarios_model->del_usuario($id_usuario);
