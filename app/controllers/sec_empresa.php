...
class Sec_empresa extends My_Controller {
    ...
    public function modificar_empresa($idEmpresa){
        $this->load->library('form_validation');
        ...
        if($this->form_validation->run()===FALSE){
                ...
                $detalle['detalle']=$this->empresas_model->get_empresa($idEmpresa);
                //$detalle['tipo_empresa']=$this->empresas_model->get_tipo_empresa();
                $this->load->view("template/header",$this->data);
    ...
    $(document).ready(function() {
        ...
        $detalle['listado']=$this->empresas_model->listado_empresas($this->session->userdata('idMaestra'),array("idEstadoproveedor"=>1));
        $this->load->view("template/header",$this->data);
        $this->load->view("mantenedores/empresa/listado",$detalle);
        $this->load->view("template/footer",$this->data);
    }
    public function detalle_empresa($idEmpresa){
        $empresa=$this->empresas_model->get_empresa($idEmpresa);
        $comuna_array=$this->common_model->get_comuna(0,$empresa['IdComuna']);
    ...
    public function eliminar_empresa($idEmpresa){
        $this->empresas_model->modificar($idEmpresa,array("idEstadoproveedor"=>0));
        redirect('listado_empresas');
    }
