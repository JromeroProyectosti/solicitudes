...
class Usuarios_model extends CI_Model{
    ...
    public function very_user($usuario, $password){
        ...
        $this->db->select("u.IdUsuario, u.NombreUsuario, u.ApellidoUsuario, u.IdTipousuario,t.NombreTipousuario");
        $this->db->from("usuarios u");
        $this->db->join("tipousuario t","t.idTipousuario=u.idTipousuario");
    ...
    public function cli_very_user($usuario, $password){
        $this->db->select("v.IdVendedor, v.NombreVendedor, u.ApellidoUsuario, u.IdTipousuario");
        $this->db->from("vendedor v");
        $this->db->where("u.UsuarioUsuario",$usuario);
        $this->db->where("u.ClaveUsuario",md5($password));
        $this->db->where("u.IdEstadousuario",1);
        $result=$this->db->get();
    ...
    public function listado_usuarios_tipo($id_maestra,$tipo){
        ...
        $this->db->join('estadousuario eu','eu.IdEstadousuario=u.idEstadousuario');
        //$this->db->join('tipoempresa','tipoempresa.IdTipoempresa=empresa.IdTipoempresa');
        $this->db->where('tu.idTipousuario',$tipo);
    ...
    public function get_acciones($id_permiso, $rut='',$campos="pa.idPermisoaccion,pa.NombrePermisoaccion"){
    ...
    public function set_permisos($rut,$array_permisos=array()){
        $array_usuario=$this->get_usuario($rut);
        if($this->input->post('checkpermisos')){
            ...
            if(is_array($array_permisos)){
                foreach($array_permisos as $row){
                    $this->db->set("idPermisoaccion",$row['idPermisoaccion']);
                    $this->db->set("idUsuario",$array_usuario['idUsuario']);
                    $this->db->insert("usuarios_permisosacciones");
